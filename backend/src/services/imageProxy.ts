...
export async function fetchExternalImage(options: {
    ...
    const cacheEnabled = config.music.imageCacheMaxGb > 0;
    if (!safeUrl) {
        return {
            ok: false,
            url,
    ...
    try {
        ...
        if (!imageResponse.ok) {
            if (imageResponse.status === 404) {
                ...
                return { ok: false, url: finalUrl, status: "not_found" };
            }
            return {
                ok: false,
                url: finalUrl,
